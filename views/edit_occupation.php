...
$occupationID = base64_decode(strrev(urldecode(explode("/", $_GET["id"])[0])));
$occupationData = $adminFunctions->getOccupationByID($occupationID)->fetch_object();
...
if ($is_post) {
    ...
    if (!$adminFunctions->existsOccupation(trim($_POST["txt_name"]), $occupationID)) {
        $isUpdated = false;
        $result = $adminFunctions->updateOccupation($occupationID, trim($_POST["txt_name"]), trim($_POST["txt_description"]));
        if ($result > 0) {
            $isUpdated = true;
            $occupationData = $adminFunctions->getOccupationByID($occupationID)->fetch_object();
        }
        ...
        if ($isUpdated) {
            array_push($msg_response["success"], "La ocupación ha sido actualizada exitosamente.");
        } else {
            array_push($msg_response["errors"], "La ocupación no ha podido ser actualizada.");
        }
    } else {
...
<head>
    ...
    <title><?= ADMIN_PANEL_NAME ?> - Editar Ocupación</title>
        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid p-0">
                    ...
                    <h1 class="h3 mb-3">Editar Ocupación</h1>
                                ...
                                <div class="card-body">
                                    <form id="edit_form" action="" method="post" novalidate="novalidate">
                                        ...
                                        <div class="mb-3 form-group">
                                            ...
                                            <input type="text" class="form-control" id="txt_name" name="txt_name" maxlength="50" value="<?= $occupationData->name ?>">
                                        </div>
                                        <div class="mb-3 form-group">
                                            <label for="txt_description" class="form-label">Descripción</label>
                                            <textarea class="form-control" id="txt_description" name="txt_description"><?= $occupationData->description ?></textarea>
                                        </div>

                                        <button type="button" class="btn btn-outline-secondary" onclick="window.location='/<?= BASE_URL ?>home';">Cancelar</button>
                                        <button type="button" class="btn btn-primary" onclick="window.location='/<?= BASE_URL ?>manage-occupations';">Regresar a la lista</button>
